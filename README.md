...
A `render.yaml` Blueprint file is included at the root of this repository. Simply connect your GitHub repo to Render, and both the backend and frontend services will be configured automatically.
...
4. Fill in the required secret environment variables (see [Environment Variables](#environment-variables)):
   ...
   - `FRONTEND_URL` — URL of the deployed frontend (e.g. `https://catmio-frontend.onrender.com`)
   - `EMAIL_HOST`, `EMAIL_USER`, `EMAIL_PASS` — SMTP credentials
   - `OBFUSCATOR_API_KEY` — obfuscator service key
   - `NEXT_PUBLIC_API_URL` — URL of the deployed backend (e.g. `https://catmio-backend.onrender.com`)
