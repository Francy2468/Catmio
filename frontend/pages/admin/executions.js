...
export default function AdminExecutions() {
  ...
  const fetchExecutions = useCallback(() => {
    setLoading(true)
    api
      .get('/api/admin/executions', { params: { page, limit, ...filters } })
      .then(({ data }) => {
  ...
  return (
    <AdminLayout>
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm mt-1">Platform-wide execution logs</p>
      </div>
                  ...
                  {executions.length === 0 ? (
                    ...
                    executions.map((ex, i) => (
                      <tr key={ex.id || i} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                        ...
                        <td className="px-4 py-3 text-gray-300 font-mono text-xs truncate max-w-[140px]">
                          {ex.hwid || '—'}
                        </td>
